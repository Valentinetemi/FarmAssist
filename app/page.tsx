    ...
    return (
      <div className="fixed inset-0 bg-white flex flex-col items-center justify-center fade-out-animation">
        <div className="flex flex-col items-center gap-6">
          ...
          <h1 className="font-serif text-4xl font-bold text-[#2d6a1a]">FarmAssist</h1>
          <p className="text-sm text-[#8a9a7a] font-satoshi">AI-powered advice for every farmer</p>
      ...
      >
        ...
        <div className="px-4 py-3 border-b border-white/10">
          ...
          <div className="flex flex-wrap gap-2">
            {['English', 'Yoruba', 'Hausa', 'Igbo', 'Français', 'Swahili'].map(lang => (
              <button
                key={lang}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          ...
          {chats.length === 0 ? (
            <p className="text-xs text-white/40 italic">Your conversations will appear here</p>
          ) : (
              ...
              {chats.map(chat => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg transition-all hover:bg-white/10 ${
                    currentChatId === chat.id ? 'bg-white/15 border-l-2 border-[#6dbe3e]' : ''
                  }`}
                >
                  <p className="text-sm font-medium line-clamp-1">{chat.title}</p>
                  <p className="text-xs text-white/50 line-clamp-1 mt-1">{chat.preview}</p>
                  <p className="text-xs text-white/30 mt-1">{formatTime(chat.timestamp)}</p>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3">
            ...
            <span className="px-2 py-1 bg-[#6dbe3e] text-[#0e1a0a] text-xs font-bold rounded">FREE</span>
          </div>
        </div>
      </aside>
        ...
        <header className="sticky top-0 bg-[#f9f8f5] border-b border-[#e5e5e0] px-6 py-4 flex items-center justify-between z-20">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-serif font-bold text-xl">
              {currentChatId
                ? chats.find(c => c.id === currentChatId)?.title || 'FarmAssist'
                : 'FarmAssist'}
            </h1>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-2 h-2 bg-[#6dbe3e] rounded-full breathe-glow"></div>
            <span className="text-sm font-medium text-[#2d6a1a]">AI Online</span>
          </div>
        </header>
          ...
          {messages.length === 0 ? (
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="mb-12 relative overflow-hidden bg-gradient-to-br from-[#2d6a1a] to-[#1e4a0e] rounded-2xl p-8 md:p-12 text-white shimmer-animation">
                ...
                <div className="relative">
                  <div className="grid md:grid-cols-2 gap-8 md:gap-12">
                    <div>
                      <p className="text-xs font-mono text-[#6dbe3e] uppercase tracking-wide mb-2">Good day —</p>
                      ...
                      <p className="text-white/80 text-base">
                        Get expert farming advice tailored to your crops, climate, and challenges.
                      </p>
                    </div>
                    <div className="flex flex-col justify-center gap-6">
                      {[
                        { label: 'Farmers', value: '33M+' },
                        { label: 'Countries', value: '190+' },
                        { label: 'Availability', value: '24/7' },
                      ].map((stat, i) => (
                        <div key={i}>
                          <p className="text-xs text-white/60 font-mono uppercase mb-1">{stat.label}</p>
              ...
              <div className="text-center mb-8">
                <p className="text-sm font-mono text-[#8a9a7a] uppercase tracking-wide">Quick Start</p>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                {[
                  { emoji: '🌱', title: 'Plan My Planting', desc: 'Get a customized planting schedule' },
                  { emoji: '🐛', title: 'Identify Pest', desc: 'Detect and treat crop diseases' },
                  { emoji: '🌦️', title: 'Weather Tips', desc: 'Get weather-based farming advice' },
                  { emoji: '💰', title: 'Market Prices', desc: 'Check current crop prices' },
                  { emoji: '🧪', title: 'Fertilizer Guide', desc: 'Learn about soil and nutrients' },
                  { emoji: '🏪', title: 'Find Supplies', desc: 'Locate farming supplies nearby' },
                ].map((card, i) => (
                  <button
                    key={i}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <span className="text-[#6dbe3e] opacity-0 group-hover:opacity-100 transition-opacity">→</span>
                    </div>
                    <h3 className="font-serif font-bold text-lg text-left mb-2">{card.title}</h3>
                    <p className="text-sm text-[#8a9a7a] text-left">{card.desc}</p>
                    <div className="mt-4 pt-4 border-t border-[#e5e5e0]">
                      <p className="text-xs font-medium text-[#2d6a1a]">Ask now →</p>
                    </div>
                  </button>
                ))}
              </div>
                  ...
                  {msg.role === 'user' ? (
                    <div className="flex justify-end gap-3">
                      <div className="max-w-xs md:max-w-md lg:max-w-lg">
                        ...
                        <p className="text-xs text-[#8a9a7a] mt-1 text-right">{formatTime(msg.timestamp)}</p>
                      </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="max-w-xs md:max-w-md lg:max-w-lg">
                        <div className="bg-white border border-[#e5e5e0] rounded-3xl rounded-tl-lg px-6 py-3 shadow-sm">
                          <p className="text-base text-[#0f1a08]">{msg.content}</p>
                        </div>
                        <p className="text-xs text-[#8a9a7a] mt-1">{formatTime(msg.timestamp)}</p>
                      </div>
                    </div>
                  )}
                </div>
